...
void irc_client_destroy(struct irc_client *irc_c) {
    ...
    free(irc_c->token);
    free(irc_c->hostname);
    free(irc_c->cert_hash);

    free(irc_c->buffer_);
    free(irc_c->ring_buffer_);
    mutex_destroy(&irc_c->ring_position_lock_);

    irc_c->channels->clear();
    delete irc_c->channels;

    thread_pool_destroy(&irc_c->tp);
}
...
bool irc_client_send_privmsg(struct irc_client *irc_c, std::string target, std::string msg) {
    ...
    msg_ss << "PRIVMSG " << target << " :" << msg;
    std::string msg_s(msg_ss.str().c_str());
    return irc_client_send_msg(irc_c, msg_s);
}
...
bool irc_client_part(struct irc_client *irc_c, std::string channel) {
    std::string part_chan = "part #" + channel;
    if (irc_client_send_msg(irc_c, part_chan)) {
        logger_write(&main_logger, LOG_LEVEL_DEBUG, "IRC_CLIENT", part_chan.c_str());
        return true;
    }
    return false;
}
...
void irc_client_parse(struct irc_client *irc_c, char* line, int len, bool irc_parse_incomplete) {
    ...
    if (!irc_parse_incomplete) {
        irc_c->irc_parse_separators = 0;
        irc_c->irc_parse_write_head = nullptr;
        if (strstr(irc_c->current_msg->command, "PRIVMSG") != nullptr || strstr(irc_c->current_msg->command, "CLEARCHAT") != nullptr || strstr(irc_c->current_msg->command, "USERSTATE") != nullptr || strstr(irc_c->current_msg->command, "ROOMSTATE") != nullptr) {
...
void irc_client_channel_remove(struct irc_client *irc_c, string channel) {
    for (int c = 0; c < irc_c->channels->size(); c++) {
        if (util_str_equals(channel.c_str(), (*irc_c->channels)[c].c_str())) {
            if (irc_c->tls_c.connected) irc_client_part(irc_c, channel);
            break;
        }
    }
}
