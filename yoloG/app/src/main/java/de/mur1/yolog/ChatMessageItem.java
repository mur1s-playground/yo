...
public class ChatMessageItem {
    ...
    public ChatMessageItem(View view, IRCMessage irc_m) {
        ...
        //this.timeView = this.view.findViewById(R.id.timeView);
        //this.badgesLayout = this.view.findViewById(R.id.badgesLayout);
        //this.nameView = this.view.findViewById(R.id.nameView);
        this.messageFlexbox = this.view.findViewById(R.id.messageFlexbox);

        clearMessageFlexbox();

        //this.timeView.setText(irc_m.display_time);
        ...
        for (int b = 0; b < irc_m.badge_replace.size(); b++) {
            ...
            if (e != null) {
                if (!e.animated) {
                    ...
                    //this.addBadgesLayoutImage((Bitmap) e.data);
                }
            } else {
                addMessageFlexboxText(b_repl.name + "/" + b_repl.version, Color.parseColor("#000000"), true, false);
                //this.addBadgesLayoutText(b_repl.name + "/" + b_repl.version);
            }
        }

        addMessageFlexboxText(irc_m.display_name, irc_m.name_color, true, true);
        //this.nameView.setText(irc_m.display_name);
        //this.nameView.setTextColor(irc_m.name_color);
            ...
            if (irc_m.emote_replace.size() == 0) {
                ...
                for (int e_r = 0; e_r < irc_m.emote_replace.size(); e_r++) {
                    ...
                    if (e_repl.range_start > message_position) {
                        addMessageFlexboxText(irc_m.message.substring(message_position, e_repl.range_start), Color.parseColor("#000000"), false, false);
                    ...
                    if (e != null) {
                        ...
                        addMessageFlexboxText(irc_m.message.substring(message_position, e_repl.range_end + 1), Color.parseColor("#000000"), false, false);
                    }
                    message_position = e_repl.range_end + 2;
                }
                if (message_position < irc_m.message.length()) {
                    addMessageFlexboxText(irc_m.message.substring(message_position), Color.parseColor("#000000"), false, false);
                }
            }
        }
    }
